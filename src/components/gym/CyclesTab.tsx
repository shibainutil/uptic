...
import { useCycles, useModules } from '../../store/gymStore';
...
export function CyclesTab() {
  ...
  async function save() {
    ...
    await add({ name: form.name.trim(), description: form.description.trim() || undefined, moduleIds: [] });
    setForm(EMPTY_FORM);
    setAddModal(false);
  }

  function confirmDelete(id: string, name: string) {
    Alert.alert('Delete Cycle', `Remove "${name}"?`, [
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.count}>{cycles.length} cycle{cycles.length !== 1 ? 's' : ''}</Text>
      ...
      {cycles.length === 0 ? (
        <View style={styles.empty}>
          ...
          <Text style={styles.emptyText}>No training cycles yet.{'\n'}Cycles combine modules into a program.</Text>
        </View>
      ) : (
        <FlatList
          data={cycles}
          ...
          renderItem={({ item }) => {
            const names = item.moduleIds
              ...
              .filter(Boolean);
            return (
              <Pressable style={styles.card} onPress={() => router.push(`/gym/cycle/${item.id}`)}>
      ...
      <Modal title="New Training Cycle" visible={addModal} onClose={() => setAddModal(false)}>
        <View style={styles.form}>
          <Input label="Name" value={form.name} onChangeText={(v) => setForm({ ...form, name: v })} placeholder="e.g. Push-Pull-Legs 6-week" autoFocus />
          <Input label="Description (optional)" value={form.description} onChangeText={(v) => setForm({ ...form, description: v })} placeholder="Notes about this cycle" />
